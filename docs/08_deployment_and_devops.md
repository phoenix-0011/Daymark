...
## مسئولیت تیم در ساخت و انتشار

| فعالیت | مسئولان |
|---|---|
| هماهنگی فنی build، نسخه‌بندی و یکپارچه‌سازی | علی ابراهیمی نیا |
| بررسی رابط در build نهایی و اندازه‌های مختلف صفحه | امیرحسین رابعی و مهدی ابراهیم زاده |
| بررسی انطباق بصری نسخه نهایی | مجتبی محمودی و مهدی نریمانی |
| بررسی migration و حفظ داده در ارتقا | علی رضا زاده |
| تست نصب و سناریوهای پذیرش Release | تمام اعضای تیم |

## 1. مدل استقرار

Daymark سرویس ابری ندارد. استقرار در نسخه فعلی به معنی تولید، بررسی و نصب خروجی Android است:

- APK برای تست مستقیم و ارائه دانشگاهی
- AAB امضاشده برای انتشار احتمالی در Google Play
...
| محیط | هدف |
|---|---|
| Development | اجرای تست‌های سریع منطق، پایگاه داده و رابط در محیط توسعه |
...
CI می‌تواند compile، test، lint و کنترل artifact را انجام دهد. ساخت Android باید در محیط کنترل‌شده و دارای Python 3.11، JDK 17، Android SDK/NDK و ابزارهای امضا انجام شود.
...
Workflow واقعی در `.github/workflows/ci.yml` قرار دارد و روی Push و Pull Request به `main` اجرا می‌شود. Gateهای مسدودکننده فعلی:

- Python Compile
- Quality Check مستقل
- Ruff correctness rules
- 42 Test خودکار
- Branch Coverage هسته با حداقل 90%

Bandit، pip-audit و Full Lint فعلاً Advisory هستند. ساخت APK در CI فعال نشده است، زیرا Toolchain Android، Wheelها و Signing نیازمند محیط کنترل‌شده‌اند. Build نهایی با [`17_release_checklist.md`](17_release_checklist.md) کنترل می‌شود.
