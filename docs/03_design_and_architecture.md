...
Daymark یک **Modular Monolith محلی** است. تمام قابلیت‌ها در یک برنامه و یک process اجرا می‌شوند، اما ماژول‌ها بر اساس مسئولیت جدا شده‌اند. Microservice برای محصول تک‌کاربره و بدون سرور، پیچیدگی غیرضروری ایجاد می‌کند.
...
مدل `Task` هسته دامنه است. زیرکارها composition هستند و با حذف Task حذف می‌شوند. Category رابطه اختیاری دارد و حذف آن باید `category_id` وظایف را NULL کند.
...
## ثبت تصمیم‌های معماری

تصمیم‌های اصلی در ADRهای زیر ثبت شده‌اند:

- [`ADR-0001-local-first-sqlite.md`](adr/ADR-0001-local-first-sqlite.md)
- [`ADR-0002-modular-monolith.md`](adr/ADR-0002-modular-monolith.md)
- [`ADR-0003-pyside6-android.md`](adr/ADR-0003-pyside6-android.md)

هر تغییر اساسی در Persistence، مرز ماژول‌ها یا Toolchain باید ADR جدید یا اصلاح ADR موجود داشته باشد.
